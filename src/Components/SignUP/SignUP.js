...
const SignUP = () => {
  ...
  const [confrimPass, setConfrimPass] = useState({ value: "", error: "" });
  ...
  const handleConfrimPass = (inputPass) => {
    if (password.value === inputPass) {
      setConfrimPass({ value: inputPass, error: "" });
    } else {
      setConfrimPass({ value: "", error: "Password din't match" });
    }
  };
  const submitFrom = (event) => {
    ...
    if (user) {
      ...
      toast.success("Thank's for Join", { id: "hello" });
    }
    if (email.value && password.value && confrimPass.value) {
          ...
          <div className="bg-white px-10 py-8 rounded-xl w-screen shadow-md max-w-sm">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  onBlur={(event) => handleConfrimPass(event.target.value)}
                  type="password"
                  className="bg-indigo-50 px-4 py-2 outline-none rounded-md w-full"
                />
                {confrimPass.error && (
            ...
            <button className="mt-3 w-full bg-cyan-500 text-white font-semibold py-2 rounded-md  tracking-wide">
              Register
            </button>
            <div>
